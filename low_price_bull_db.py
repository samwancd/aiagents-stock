...
def init_db():
    ...
    try:
        ...
        c = conn.cursor()
        c.execute('''
            CREATE TABLE IF NOT EXISTS low_price_bull_analysis (
...
def save_analysis(stocks_df, top_n):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            'INSERT INTO low_price_bull_analysis (timestamp, params, result, stocks_data) VALUES (?, ?, ?, ?)',
            (
...
def get_history_list():
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT id, timestamp, params, result FROM low_price_bull_analysis ORDER BY id DESC')
        rows = c.fetchall()
        conn.close()
...
def get_analysis_result(record_id):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('SELECT stocks_data, timestamp, params FROM low_price_bull_analysis WHERE id = ?', (record_id,))
        row = c.fetchone()
        conn.close()
...
def delete_analysis_record(record_id):
    ...
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('DELETE FROM low_price_bull_analysis WHERE id = ?', (record_id,))
        conn.commit()
        conn.close()
        return True
    except Exception as e:
